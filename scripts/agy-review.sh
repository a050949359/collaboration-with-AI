...
1. **每條發現都必須引用 diff 中真實存在的那一行**（在發現裡用 \`> 原文\` 貼出來）。
   引不出原文 = 你在猜，這條不准寫。
2. **檔名需與 diff 完全一致**：照 diff 的 \`+++ b/<path>\` 完整路徑，**不可縮寫或省略
   目錄層級**（例如不可把 \`pages/Admin/System.vue\` 寫成 \`pages/System.vue\`）。
   **行號用 \`@@ -a,b +c,d @@\` hunk header 目測估算即可**（從該行往下數幾行），
   是給人對照用的參考位置、不是精確座標，差一兩行沒關係，**不需要也不准為了算行號寫程式或執行任何腳本**。
