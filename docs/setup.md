...
舊版 `import:countries`／`import:cities` artisan 指令與對應的 `Aviation\Country`/`City` 功能已退役。國家與行政區資料改由 `POST /api/mcp/territory`（`territory:mcp` scope，admin-only）管理，見 CLAUDE.md「MCP Server 使用規範」。
