...
async function submit() {
    ...
    try {
        ...
        if (error instanceof AuthApiError) {
            ...
            generalError.value = t('auth.submit_register') + ' 失敗，請稍後再試。';
        }
    } finally {
...
<template>
    <form class="space-y-5" @submit.prevent="submit">
        <div class="space-y-1.5">
            <label class="binary-label block text-[11px] font-bold text-[var(--binary-outline)] uppercase" for="rf-name">
                {{ t('auth.label_name') }}
            </label>
            <input
                id="rf-name"
            ...
            />
            <p v-if="fieldErrors.name?.length" class="text-xs text-[var(--binary-tertiary)]">
                {{ fieldErrors.name[0] }}
            </p>
        </div>

        <div class="space-y-1.5">
            <label class="binary-label block text-[11px] font-bold text-[var(--binary-outline)] uppercase" for="rf-email">
                {{ t('auth.label_email') }}
            </label>
            <input
                id="rf-email"
            ...
            />
            <p v-if="fieldErrors.email?.length" class="text-xs text-[var(--binary-tertiary)]">
                {{ fieldErrors.email[0] }}
            </p>
        </div>

        <div class="space-y-1.5">
            <label class="binary-label block text-[11px] font-bold text-[var(--binary-outline)] uppercase" for="rf-password">
            ...
            <div class="relative">
                ...
                >
                    <svg v-if="showPwd.password" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <p v-if="fieldErrors.password?.length" class="text-xs text-[var(--binary-tertiary)]">
                {{ fieldErrors.password[0] }}
            </p>
        </div>

        <div class="space-y-1.5">
            <label class="binary-label block text-[11px] font-bold text-[var(--binary-outline)] uppercase" for="rf-password-confirm">
            ...
            <div class="relative">
                ...
                >
                    <svg v-if="showPwd.confirm" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                    </svg>
                    <svg v-else class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        ...
        <p v-if="fieldErrors.terms?.length" class="text-xs text-[var(--binary-tertiary)]">
            {{ fieldErrors.terms[0] }}
        </p>
        ...
        <button class="binary-button" :disabled="isSubmitting" type="submit">
            {{ isSubmitting ? t('auth.submitting_register') : t('auth.submit_register') }}
            <span aria-hidden="true">-></span>
        </button>

        <div v-if="turnstileEnabled" class="mt-2">
            <Turnstile v-model="form.cf_turnstile_response" />
            <p v-if="fieldErrors.cf_turnstile_response?.length" class="mt-1 text-sm text-[var(--binary-tertiary)]">
