...
import { tourFetch, safeParseParams } from '@/lib/tour-api';
import type {
    Tour,
    Booking,
    ExportTask,
...
async function loadTourBookings(tourId: number) {
    if (tourBookingsCache.value[tourId]) {
        return;
    }
    ...
    try {
        tourBookingsCache.value[tourId] = await tourFetch<Booking[]>(
            `/bookings?tour_id=${tourId}`,
        );
    } finally {
...
function toggleTourExpand(tourId: number) {
    if (expandedTourId.value === tourId) {
        expandedTourId.value = null;
...
function startPolling(id: number) {
    if (pollingMap.has(id)) {
        return;
    }

    const timer = setInterval(async () => {
        const data = await tourFetch<{ status: string }>(
            `/exports/${id}/status`,
        ).catch(() => null);
...
async function triggerExport(tour: Tour) {
    ...
    try {
        ...
        exportError.value =
            e instanceof Error
                ? e.message
                : t('tour_playground.mgmt.export_failed');
    } finally {
...
async function loadMgmtTours() {
    ...
    try {
        mgmtTours.value = await tourFetch<Tour[]>('/tours');
    } finally {
        mgmtToursLoading.value = false;
    }
}
...
function cancelTourForm() {
    ...
    flightForm.value = {
        flight_number: '',
        cabin_class: cabinClasses.value[0],
        origin_airport_id: 0,
    ...
    hotelForm.value = {
        ...
        check_in_date: '',
        check_out_date: '',
        room_type: roomTypes.value[0],
        number_of_rooms: 1,
        cost_price_per_night: 0,
        remarks: '',
    };
    originQuery.value = '';
    destQuery.value = '';
...
async function submitTour() {
    ...
    try {
        const payload = {
            ...tourForm.value,
            selling_price: parseFloat(tourForm.value.selling_price),
        };

        if (tourFormMode.value === 'create') {
            await tourFetch('/tours', {
                method: 'POST',
                body: JSON.stringify(payload),
            });
            tourFormSuccess.value = t(
                'tour_playground.mgmt.tour_create_success',
            );
        } else if (editingTour.value) {
            await tourFetch(`/tours/${editingTour.value.id}`, {
                method: 'PUT',
                body: JSON.stringify(payload),
            });
            tourFormSuccess.value = t(
                'tour_playground.mgmt.tour_update_success',
            );
        }
        ...
        tourFormError.value =
            e instanceof Error
                ? e.message
                : t('tour_playground.mgmt.op_failed');
    } finally {
...
function onOriginInput() {
    ...
    originTimer = setTimeout(async () => {
        originResults.value = await searchAirports(originQuery.value);
    }, 300);
}
...
function onDestInput() {
    ...
    destTimer = setTimeout(async () => {
        destResults.value = await searchAirports(destQuery.value);
    }, 300);
}
...
async function loadEditingTourFlights() {
    ...
    try {
        editingTourFlights.value = await tourFetch<TourFlight[]>(
            `/${editingTour.value.id}/flights`,
        );
    } finally {
...
async function addFlight() {
    if (!editingTour.value || flightSubmitting.value) {
        return;
    }
    ...
    try {
        await tourFetch(`/${editingTour.value.id}/flights`, {
            method: 'POST',
            body: JSON.stringify(flightForm.value),
        });
        flightForm.value = {
            flight_number: '',
            cabin_class: cabinClasses.value[0],
        ...
        flightFormError.value =
            e instanceof Error
                ? e.message
                : t('tour_playground.mgmt.add_failed');
    } finally {
...
async function deleteFlight(flightId: number) {
    ...
    await tourFetch(`/${editingTour.value.id}/flights/${flightId}`, {
        method: 'DELETE',
    }).catch(() => null);
    editingTourFlights.value = editingTourFlights.value.filter(
        (f) => f.id !== flightId,
    );
}
...
async function loadEditingTourHotels() {
    ...
    try {
        editingTourHotels.value = await tourFetch<TourHotel[]>(
            `/${editingTour.value.id}/hotels`,
        );
    } finally {
...
async function addHotel() {
    if (!editingTour.value || hotelSubmitting.value) {
        return;
    }
    ...
    try {
        await tourFetch(`/${editingTour.value.id}/hotels`, {
            method: 'POST',
...
async function deleteHotel(hotelId: number) {
    ...
    await tourFetch(`/${editingTour.value.id}/hotels/${hotelId}`, {
        method: 'DELETE',
    }).catch(() => null);
    editingTourHotels.value = editingTourHotels.value.filter(
        (h) => h.id !== hotelId,
    );
}
...
watch(
    ...
    (isActive) => {
        if (isActive) {
            if (!exports_.value.length) {
                loadExports();
            }

            if (!mgmtTours.value.length) {
                loadMgmtTours();
            }
        } else {
            stopAllPolling();
    ...
    <div>
        ...
        <div class="mb-4 flex gap-1">
            <button
                v-for="tab in [
                    {
                        key: 'bookings',
                        label: t('tour_playground.mgmt.sub_bookings'),
                    },
                    {
                        key: 'exports',
                        label: t('tour_playground.mgmt.sub_exports'),
                    },
                    {
                        key: 'tours',
                        label: t('tour_playground.mgmt.sub_tours'),
                    },
                ]"
                :key="tab.key"
        ...
        <div v-show="mgmtTab === 'bookings'" class="binary-card-raised">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button
                    ...
                    @click="loadMgmtTours"
                >
                    {{ t('tour_playground.mgmt.refresh') }}
                </button>
            </div>
            <p v-if="exportError" class="mb-3 text-xs text-red-400">
                {{ exportError }}
            </p>
            <p
                v-if="mgmtToursLoading"
            ...
            <div v-else class="space-y-1.5">
                ...
                >
                    ...
                    >
                        <span
                            class="w-2 shrink-0 text-[10px] text-[var(--binary-text-muted)]"
                        >
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-1.5">
                                ...
                                >
                                    {{
                                        tour.is_formed
                                            ? t('tour_playground.mgmt.formed')
                                            : t(
                                                  'tour_playground.mgmt.not_formed',
                                              )
                                    }}
                                </span>
                                <span
                                ...
                                >
                                    {{
                                        t('tour_playground.mgmt.overbooked', {
                                            n:
                                                (tour.booked_pax ?? 0) -
                                                tour.max_pax,
                                        })
                        ...
                        <span
                            class="shrink-0 text-xs text-[var(--binary-text-muted)]"
                        >
                        ...
                        >
                            {{
                                exportingId === tour.id
                                    ? t('tour_playground.mgmt.exporting')
                                    : t('tour_playground.mgmt.export_btn')
                            }}
                        </button>
                    </div>
                    ...
                    >
                        <p
                            v-if="loadingTourId === tour.id"
                            class="py-3 text-center text-xs text-[var(--binary-text-muted)]"
                        >
                            {{ t('common.loading') }}
                        </p>
                        <template v-else-if="tourBookingsCache[tour.id]">
                            <p
                                v-if="!tourBookingsCache[tour.id].length"
                                class="py-3 text-center text-xs text-[var(--binary-text-muted)]"
                            >
                                {{ t('tour_playground.mgmt.no_bookings') }}
                            </p>
                            <table v-else class="w-full">
                                <thead>
                                    <tr
                                        class="text-left text-[10px] text-[var(--binary-text-muted)] uppercase"
                                    >
                                        <th class="pr-3 pb-1.5">
                                            {{
                                                t(
                                                    'tour_playground.mgmt.col_booking_ref',
                                                )
                                            }}
                                        </th>
                                        <th class="pr-3 pb-1.5">
                                            {{
                                                t(
                                                    'tour_playground.mgmt.col_passenger',
                                                )
                                            }}
                                        </th>
                                        <th class="pr-3 pb-1.5">
                                            {{
                                                t(
                                                    'tour_playground.mgmt.col_travelers',
                                                )
                                            }}
                                        </th>
                                        <th class="pr-3 pb-1.5">
                                            {{
                                                t(
                                                    'tour_playground.mgmt.col_status',
                                                )
                                            }}
                                        </th>
                                        <th class="pb-1.5">
                                ...
                                <tbody>
                                    <tr
                                        v-for="b in tourBookingsCache[tour.id]"
                                        :key="b.id"
        ...
        <div v-show="mgmtTab === 'exports'" class="space-y-4">
            <div class="binary-card-raised">
                ...
                <p
                    v-if="exportsLoading"
                    class="py-4 text-center text-sm text-[var(--binary-text-muted)]"
                >
                    {{ t('common.loading') }}
                </p>
                <div v-else class="space-y-2">
                    <p
                        v-if="!exports_.length"
                        class="py-4 text-center text-sm text-[var(--binary-text-muted)]"
                    >
                        {{ t('tour_playground.mgmt.no_exports') }}
                    </p>
                    <div
                        v-for="e in exports_"
                        :key="e.id"
                    ...
                    >
                        <div>
                            <p class="font-mono text-xs">
                                {{ safeParseParams(e.params).tour_code ?? '-' }}
                            </p>
                            <p
                                class="mt-0.5 text-[10px] text-[var(--binary-text-muted)]"
                            >
                                {{ e.created_at }}
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            >
                                {{ t('tour_playground.mgmt.download') }}
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div v-show="mgmtTab === 'tours'" class="space-y-4">
            ...
            <div v-if="!showTourForm" class="binary-card-raised">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        >
                            {{ t('tour_playground.mgmt.add_tour') }}
                        </button>
                    </div>
                </div>
                <p
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr
                                class="border-b border-[var(--binary-outline)]/20 text-left text-[10px] text-[var(--binary-text-muted)] uppercase"
                            >
                                <th class="pr-4 pb-2">Code</th>
                                <th class="pr-4 pb-2">
                                    {{ t('tour_playground.mgmt.col_name') }}
                                </th>
                                <th class="pr-4 pb-2">
                                    {{ t('tour_playground.mgmt.col_depart') }}
                                </th>
                                <th class="pr-4 pb-2">
                                    {{ t('tour_playground.mgmt.col_price') }}
                                </th>
                                <th class="pr-4 pb-2">
                                    {{
                                        t('tour_playground.mgmt.col_pax_limit')
                                    }}
                                </th>
                                <th class="pb-2"></th>
                            </tr>
                        ...
                        <tbody>
                            <tr
                                ...
                                :class="
                                    editingTour?.id === tour.id && showTourForm
                            ...
                            >
                                ...
                                <td
                                    class="py-2 pr-4 text-xs text-[var(--binary-text-muted)]"
                                >
                                    {{ tour.departure_date }}
                                </td>
                                <td class="py-2 pr-4 text-xs">
            ...
            <div v-if="showTourForm" class="binary-card-raised">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-bold tracking-wide uppercase">
                            {{
                                tourFormMode === 'create'
                                    ? t('tour_playground.mgmt.create_title')
                                    : editingTour?.name
                            }}
                        </h2>
                        <p
                            v-if="tourFormMode === 'edit'"
                    ...
                    <button
                        class="binary-ghost-button px-3 py-1 text-xs"
                        @click="cancelTourForm"
                    >
                        {{ t('tour_playground.mgmt.cancel') }}
                    </button>
                </div>
                ...
                <div
                    v-if="tourFormMode === 'edit'"
                    class="mb-5 flex border-b border-[var(--binary-outline)]/20"
                >
                    <button
                        v-for="tab in [
                            {
                                key: 'info',
                                label: t('tour_playground.mgmt.edit_tab_info'),
                            },
                            {
                                key: 'flights',
                                label: t(
                                    'tour_playground.mgmt.edit_tab_flights',
                                ),
                            },
                            {
                                key: 'hotels',
                                label: t(
                                    'tour_playground.mgmt.edit_tab_hotels',
                                ),
                            },
                        ]"
                        :key="tab.key"
                        class="-mb-[2px] border-b-2 px-4 py-2 text-xs font-medium tracking-wide uppercase transition-colors"
                        :class="
                            tourEditTab === tab.key
                ...
                <!-- 基本資料 -->
                <div
                    v-show="tourFormMode === 'create' || tourEditTab === 'info'"
                >
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label
                                ...
                                >{{
                                    t('tour_playground.mgmt.form_name')
                                }}</label
                            >
                            <input
                                ...
                                :placeholder="
                                    t(
                                        'tour_playground.mgmt.placeholder_tour_name',
                                    )
                                "
                        ...
                        <div>
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_type')
                                }}</label
                            >
                            <select
                                v-model="tourForm.type"
                                class="binary-input w-full"
                            >
                        ...
                        <div>
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_duration')
                                }}</label
                            >
                            <input
                                v-model.number="tourForm.duration"
                                type="number"
                                min="1"
                                class="binary-input w-full"
                            />
                        ...
                        <div>
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_depart')
                                }}</label
                            >
                            <input
                                v-model="tourForm.departure_date"
                                type="date"
                                class="binary-input w-full"
                            />
                        </div>
                        <div>
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_return')
                                }}</label
                            >
                            <input
                                v-model="tourForm.return_date"
                                type="date"
                                class="binary-input w-full"
                            />
                        </div>
                        <div>
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_price')
                                }}</label
                            >
                            <input
                                v-model="tourForm.selling_price"
                                type="number"
                                min="0"
                                class="binary-input w-full"
                            />
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.form_min_pax')
                                    }}</label
                                >
                                <input
                                    v-model.number="tourForm.min_pax"
                                    type="number"
                                    min="1"
                                    class="binary-input w-full"
                                />
                            ...
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.form_max_pax')
                                    }}</label
                                >
                                <input
                                    v-model.number="tourForm.max_pax"
                                    type="number"
                                    min="1"
                                    class="binary-input w-full"
                                />
                        ...
                        <div class="sm:col-span-2">
                            <label
                                class="binary-label mb-1 block text-[10px] uppercase"
                                >{{
                                    t('tour_playground.mgmt.form_remarks')
                                }}</label
                            >
                            <textarea
                                v-model="tourForm.remarks"
                                rows="2"
                    ...
                    <div class="mt-4 flex items-center gap-3">
                        ...
                        >
                            {{
                                tourFormLoading
                                    ? t('tour_playground.mgmt.processing')
                                    : tourFormMode === 'create'
                                      ? t('tour_playground.mgmt.create')
                                      : t('tour_playground.mgmt.update')
                            }}
                        </button>
                        <p v-if="tourFormError" class="text-xs text-red-400">
                            {{ tourFormError }}
                        </p>
                        <p
                            v-if="tourFormSuccess"
                ...
                <div
                    v-show="
                        tourFormMode === 'edit' && tourEditTab === 'flights'
                    "
                >
                    <p
                        v-if="flightsLoading"
                        class="mb-3 text-xs text-[var(--binary-text-muted)]"
                    >
                        {{ t('common.loading') }}
                    </p>
                    <div
                        v-else-if="editingTourFlights.length"
                    ...
                    >
                        ...
                        >
                            <div>
                                ...
                                <span
                                    class="ml-2 text-[var(--binary-text-muted)]"
                                    >{{ f.cabin_class }}</span
                                >
                                <p
                                    class="mt-0.5 text-[var(--binary-text-muted)]"
                                >
                                    {{ f.departure_time }} →
                                    {{ f.arrival_time }}
                                </p>
                            ...
                            <button
                                class="binary-ghost-button px-2 py-0.5 text-[10px] text-red-400 hover:text-red-300"
                                @click="deleteFlight(f.id)"
                            >
                                {{ t('tour_playground.mgmt.delete') }}
                            </button>
                        </div>
                    </div>
                    <p
                        v-else-if="!flightsLoading"
                        class="mb-4 text-xs text-[var(--binary-text-muted)]"
                    >
                        {{ t('tour_playground.mgmt.no_flights') }}
                    </p>

                    <template v-if="canAddFlight">
                        <p
                            class="mb-3 text-[10px] tracking-wide text-[var(--binary-text-muted)] uppercase"
                        >
                            {{ t('tour_playground.mgmt.add_flight_title') }}
                        </p>
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            <div>
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_flight_no',
                                        )
                                    }}</label
                                >
                                <input
                            ...
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_cabin')
                                    }}</label
                                >
                                <select
                                    v-model="flightForm.cabin_class"
                                    class="binary-input w-full"
                                >
                                    <option
                                        v-for="v in cabinClasses"
                                        :key="v"
                                        :value="v"
                                    >
                                        {{
                                            t(`tour_playground.mgmt.cabin_${v}`)
                                        }}
                                    </option>
                                </select>
                            </div>
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_cost')
                                    }}</label
                                >
                                <input
                                    v-model.number="flightForm.cost_price"
                                    type="number"
                                    min="0"
                                    class="binary-input w-full"
                                />
                            ...
                            <div class="relative">
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_origin')
                                    }}</label
                                >
                                <input
                                    v-model="originQuery"
                                    type="text"
                                    :placeholder="
                                        t('tour_playground.mgmt.search_airport')
                                    "
                                    class="binary-input w-full"
                                    @input="onOriginInput"
                                    @blur="
                                ...
                                />
                                <ul
                                    v-if="originOpen && originResults.length"
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            class="ml-1.5 text-[var(--binary-text-muted)]"
                                            >{{ a.name }}</span
                                        >
                                    </li>
                                </ul>
                            ...
                            <div class="relative">
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_dest')
                                    }}</label
                                >
                                <input
                                    v-model="destQuery"
                                ...
                                />
                                <ul
                                    v-if="destOpen && destResults.length"
                                    class="absolute z-10 mt-1 w-full overflow-hidden rounded-lg border border-[var(--binary-outline)]/30 bg-[var(--binary-surface)] shadow-lg"
                                >
                                    <li
                                        v-for="a in destResults"
                                        :key="a.id"
                                        class="cursor-pointer px-3 py-2 text-xs hover:bg-[var(--binary-primary)]/10"
                            ...
                            <div>
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_depart_time',
                                        )
                                    }}</label
                                >
                                <input
                            ...
                            <div>
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_arrive_time',
                                        )
                                    }}</label
                                >
                                <input
                        ...
                        <p
                            v-if="flightFormError"
                            class="mt-2 text-xs text-red-400"
                        >
                            {{ flightFormError }}
                        </p>
                        <button
                            class="binary-button mt-3"
                            :disabled="flightSubmitting"
                            @click="addFlight"
                        >
                            {{
                                flightSubmitting
                                    ? t('tour_playground.mgmt.adding')
                                    : t('tour_playground.mgmt.add_flight_btn')
                            }}
                        </button>
                    </template>
                    <p v-else class="text-xs text-[var(--binary-text-muted)]">
                        {{ t('tour_playground.mgmt.flights_max_reached') }}
                    </p>
                </div>

                <!-- 飯店 -->
                <div
                    v-show="tourFormMode === 'edit' && tourEditTab === 'hotels'"
                >
                    <p
                        v-if="hotelsLoading"
                        class="mb-3 text-xs text-[var(--binary-text-muted)]"
                    ...
                    <div
                        v-else-if="editingTourHotels.length"
                        class="mb-4 space-y-1.5"
                    >
                        <div
                        ...
                        >
                            <div>
                                ...
                                <span
                                    class="ml-2 text-[var(--binary-text-muted)]"
                                    >{{ h.room_type }} ×
                                    {{ h.number_of_rooms }} 間</span
                                >
                                <p
                                    class="mt-0.5 text-[var(--binary-text-muted)]"
                                >
                                    {{ h.check_in_date }} –
                                    {{ h.check_out_date }}（{{ h.nights }} 晚）
                                    ｜ NT$
                                    {{
                                        Number(
                                            h.total_cost_price,
                    ...
                    <p
                        v-else-if="!hotelsLoading"
                        class="mb-4 text-xs text-[var(--binary-text-muted)]"
                    >
                        {{ t('tour_playground.mgmt.no_hotels') }}
                    </p>
                    ...
                    <template v-if="canAddHotel">
                        <p
                            class="mb-3 text-[10px] tracking-wide text-[var(--binary-text-muted)] uppercase"
                        >
                            {{ t('tour_playground.mgmt.add_hotel_title') }}
                        </p>
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            <div class="sm:col-span-2">
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_hotel_name',
                                        )
                                    }}</label
                                >
                                <input
                                    v-model="hotelForm.hotel_name"
                                    type="text"
                                    :placeholder="
                                        t(
                                            'tour_playground.mgmt.placeholder_hotel_name',
                                        )
                                    "
                                    class="binary-input w-full"
                                />
                            ...
                            <div>
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_room_type',
                                        )
                                    }}</label
                                >
                                <select
                                    v-model="hotelForm.room_type"
                                    class="binary-input w-full"
                                >
                                    <option
                                    ...
                                    >
                                        {{
                                            t(`tour_playground.mgmt.room_${v}`)
                                        }}
                                    </option>
                                </select>
                            </div>
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_rooms')
                                    }}</label
                                >
                                <input
                                    v-model.number="hotelForm.number_of_rooms"
                                    type="number"
                                    min="1"
                                    class="binary-input w-full"
                                />
                            ...
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_checkin')
                                    }}</label
                                >
                                <input
                                    v-model="hotelForm.check_in_date"
                                    type="date"
                                    class="binary-input w-full"
                                />
                            </div>
                            <div>
                                <label
                                    class="binary-label mb-1 block text-[10px] uppercase"
                                    >{{
                                        t('tour_playground.mgmt.label_checkout')
                                    }}</label
                                >
                                <input
                                    v-model="hotelForm.check_out_date"
                                    type="date"
                                    class="binary-input w-full"
                                />
                            </div>
                            <div>
                                <label
                                    ...
                                    >{{
                                        t(
                                            'tour_playground.mgmt.label_cost_per_night',
                                        )
                                    }}</label
                                >
                                <input
                                    v-model.number="
                                        hotelForm.cost_price_per_night
                                    "
                                    type="number"
                                    min="0"
                                    class="binary-input w-full"
                        ...
                        >
                            {{
                                hotelSubmitting
                                    ? t('tour_playground.mgmt.adding')
                                    : t('tour_playground.mgmt.add_hotel_btn')
                            }}
                        </button>
                    </template>
                    <p v-else class="text-xs text-[var(--binary-text-muted)]">
                        {{
                            t('tour_playground.mgmt.hotels_full', {
                                n: editingTour?.duration,
                            })
                        }}
                    </p>
                </div>
