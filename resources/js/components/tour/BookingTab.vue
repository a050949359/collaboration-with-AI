...
import { tourFetch, type Tour } from '@/lib/tour-api';
...
async function lookupBooker() {
    ...
    else { bookingForm.value.passenger_id = ''; bookerError.value = t('tour_playground.booking.not_registered'); }
}
...
async function lookupCompanion(i: number) {
    ...
    else { bookingForm.value.companions[i] = ''; companionErrors.value[i] = t('tour_playground.booking.not_registered'); }
}
...
async function createPassenger(target: 'booker' | number) {
    ...
    try {
        ...
        newPassengerError.value = e instanceof Error ? e.message : t('tour_playground.booking.add_failed');
    } finally {
...
const passengerConflicts = computed(() => {
    ...
    return bookingForm.value.companions.map((id, i) => {
        if (id && id === bookerId) return t('tour_playground.booking.conflict_same_booker', { n: i + 1 });
        const dupIdx = bookingForm.value.companions.findIndex((c, j) => j < i && c === id && id !== '');
        if (dupIdx !== -1) return t('tour_playground.booking.conflict_duplicate', { n: i + 1, m: dupIdx + 1 });
        return null;
    }).filter(Boolean);
});
...
async function submitBooking() {
    ...
    try {
        ...
        bookingError.value = e instanceof Error ? e.message : t('tour_playground.booking.create_failed');
    } finally {
...
watch(() => props.active, (isActive) => {
    if (isActive && !tours.value.length) loadTours();
});
</script>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="binary-card-raised rounded-2xl">
            <div class="mb-4 flex items-center justify-between">
                <h2 class="text-sm font-bold uppercase tracking-wide">{{ t('tour_playground.booking.select_tour') }}</h2>
            ...
            <p v-if="toursLoading" class="py-6 text-center text-sm text-[var(--binary-text-muted)]">{{ t('common.loading') }}</p>
            <div
                ref="tourListRef"
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            >{{ tour.is_formed ? t('tour_playground.booking.formed') : t('tour_playground.booking.not_formed') }}</span>
                            <span v-if="(tour.booked_pax ?? 0) > tour.max_pax"
                                ...
                                {{ t('tour_playground.booking.overbooked', { n: (tour.booked_pax ?? 0) - tour.max_pax }) }}
                            </span>
                    ...
                    <p class="mt-0.5 text-xs text-[var(--binary-text-muted)]">
                        {{ tour.departure_date }} ｜ {{ tour.duration }} {{ t('tour_playground.days_unit') }} ｜
        ...
        <!-- Booking Form -->
        <div class="binary-card-raised rounded-2xl">
            <h2 class="mb-4 text-sm font-bold uppercase tracking-wide">{{ t('tour_playground.booking.booking_info') }}</h2>
            ...
            <template v-else>
                ...
                <div class="space-y-4">
                    <!-- 付款人 -->
                    <div>
                        <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_booker') }}</label>
                        <div class="flex gap-2">
                            <input
                                v-model="bookerName"
                                type="email"
                                :placeholder="t('tour_playground.booking.email_placeholder')"
                                class="binary-input flex-1"
                                @blur="lookupBooker"
                            />
                            <button class="binary-ghost-button px-3 text-xs" @click="randomizeBooker">{{ t('tour_playground.booking.random') }}</button>
                        </div>
                        <div v-if="bookerError" class="mt-2 space-y-2">
                            <p class="text-xs text-yellow-400">
                                ...
                                >{{ t('tour_playground.booking.add_passenger') }}</button>
                            </p>
                            <div v-if="newPassengerTarget === 'booker'" class="rounded-lg border border-[var(--binary-outline)]/20 p-3 space-y-2">
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_name') }}</label>
                                        <input v-model="newPassengerForm.name" type="text" :placeholder="t('tour_playground.booking.placeholder_name')" class="binary-input w-full" />
                                    </div>
                                    <div>
                                        <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_phone') }}</label>
                                        <input v-model="newPassengerForm.phone" type="tel" :placeholder="t('tour_playground.booking.placeholder_phone')" class="binary-input w-full" />
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-2">
                                    <button class="binary-button px-3 py-1 text-xs" :disabled="newPassengerLoading" @click="createPassenger('booker')">
                                        {{ newPassengerLoading ? t('tour_playground.booking.adding') : t('tour_playground.booking.confirm_add') }}
                                    </button>
                                    <button class="binary-ghost-button px-3 py-1 text-xs" @click="newPassengerTarget = null">{{ t('tour_playground.booking.cancel') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-6 gap-3">
                        <div class="col-span-1">
                            <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_travelers') }}</label>
                        ...
                        >
                            <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.companion_n', { n: i + 1 }) }}</label>
                            <div class="flex gap-2">
                                <input
                                ...
                                />
                                <button class="binary-ghost-button px-3 text-xs" @click="randomizeCompanion(i)">{{ t('tour_playground.booking.random') }}</button>
                            </div>
                    ...
                    <div>
                        <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_discount') }}</label>
                        <input v-model.number="bookingForm.discount_amount" type="number" min="0" class="binary-input w-full" />
                    </div>

                    <div>
                        <label class="binary-label mb-1 block text-[10px] uppercase">{{ t('tour_playground.booking.label_remarks') }}</label>
                    ...
                    <div class="rounded-lg border border-[var(--binary-outline)]/20 px-4 py-3">
                        <p class="text-[10px] uppercase text-[var(--binary-text-muted)]">{{ t('tour_playground.booking.due_amount') }}</p>
                    ...
                    <button
                        ...
                        @click="submitBooking"
                    >{{ bookingLoading ? t('tour_playground.booking.submitting') : t('tour_playground.booking.submit') }}</button>
                </div>
            </template>
        </div>
