...
import type { Passenger } from '@/lib/tour-api';
...
async function loadRandomPassenger() {
    ...
    try {
        const qs = passengerFilter.value
            ? `?filter=${passengerFilter.value}`
            : '';
        ...
        passengerError.value =
            e instanceof Error
                ? e.message
                : t('tour_playground.passenger.load_failed');
    } finally {
...
<template>
    <div class="binary-card-raised">
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            <button
                class="binary-button ml-auto px-4 py-1.5 text-xs"
                :disabled="passengerLoading"
                @click="loadRandomPassenger"
            >
                {{
                    passengerLoading
                        ? t('common.loading')
                        : t('tour_playground.passenger.random_pick')
                }}
            </button>
        </div>
        ...
        <!-- Empty state -->
        <p
            v-if="!passenger && !passengerLoading && !passengerError"
        ...
        <p v-else-if="passengerError" class="text-sm text-red-400">
            {{ passengerError }}
        </p>
        ...
        <template v-else-if="passenger">
            <div
                class="rounded-xl border border-[var(--binary-outline)]/20 p-4"
            >
                ...
                <template
                    v-if="
                        passengerFilter === 'booker' &&
                        passenger.bookings?.length
                    "
                >
                    <p
                        class="mb-2 text-[10px] text-[var(--binary-text-muted)] uppercase"
                    >
                        {{
                            t('tour_playground.passenger.bookings_label', {
                                n: passenger.bookings.length,
                            })
                        }}
                    </p>
                    <div class="space-y-2">
                        ...
                        >
                            ...
                            <p class="mt-0.5 text-[var(--binary-text-muted)]">
                                ...
                                {{ t('tour_playground.pax_unit') }} ｜ NT$
                                {{ Number(b.final_amount).toLocaleString() }}
                            </p>
                            <p class="mt-0.5 text-[var(--binary-text-muted)]">
                                {{
                                    t('tour_playground.passenger.paid_prefix')
                                }}NT$
                                {{
                                    b.payments
                                        ?.reduce(
                                ...
                                <span
                                    ...
                                    "
                                    class="ml-1 text-[var(--binary-primary)]"
                                    >{{
                                        t('tour_playground.passenger.paid_full')
                                    }}</span
                                >
                            </p>
                        </div>
                ...
                <template
                    v-else-if="
                        passengerFilter === 'companion_only' &&
                        passenger.companion_of?.length
                    "
                >
                    <p
                        class="mb-2 text-[10px] text-[var(--binary-text-muted)] uppercase"
                    >
                        {{
                            t('tour_playground.passenger.companions_label', {
                                n: passenger.companion_of.length,
                            })
                        }}
                    </p>
                ...
                <template v-else-if="passengerFilter === 'no_booking'">
                    <p class="text-xs text-[var(--binary-text-muted)]">
                        {{ t('tour_playground.passenger.no_records') }}
                    </p>
                </template>
                ...
                <template v-else-if="passengerFilter === ''">
                        ...
                        <div class="mb-4 space-y-2">
                            ...
                            >
                                ...
                                >
                                    {{
                                        t(
                                            'tour_playground.passenger.paid_prefix',
                                        )
                                    }}NT$
                                    {{
                                        b.payments
                                    ...
                                    <span
                                        ...
                                        >{{
                                            t(
                                                'tour_playground.passenger.paid_full',
                                            )
                                        }}</span
                                    >
                                </p>
                    ...
                    <template v-if="passenger.companion_of?.length">
                        ...
                        >
                            {{
                                t(
                                    'tour_playground.passenger.companions_label',
                                    { n: passenger.companion_of.length },
                                )
                            }}
                        </p>
