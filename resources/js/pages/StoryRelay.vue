...
function updateCountdown() {
    ...
    if (!ts || selected.value?.status !== 'active') {
        countdown.value = '';
    ...
    if (diff <= 0) {
        countdown.value = t('story_relay.countdown_imminent');
...
const worldStateSummary = computed(() => {
    ...
    return worldStateExpanded.value
        ? ws
        : ws.length > 200
          ? ws.slice(0, 200) + '…'
          : ws;
...
async function loadSessions() {
    ...
    try {
        sessions.value = await fetchJson<SessionListItem[]>(
            api.story.sessions(),
        );
    } catch (e: unknown) {
        error.value =
            e instanceof Error ? e.message : t('story_relay.err_load');
    } finally {
...
async function generateDraft() {
    ...
    try {
        const res = await fetchJson<{ setup: SetupDraft }>(
            api.story.setupGenerate(),
            {
                method: 'POST',
...
async function refineDraft() {
    ...
    try {
        ...
        setupError.value =
            e instanceof Error ? e.message : t('story_relay.err_ai_refine');
    } finally {
        setupLoading.value = false;
    }
}
...
async function createSession() {
    if (!setupDraft.value || !setupTitle.value.trim()) {
        return;
    }
    ...
    try {
        const body = {
            ...
            setting: {
                world: setupDraft.value.world,
                opening: setupDraft.value.opening,
            },
        ...
        setupError.value =
            e instanceof Error ? e.message : t('story_relay.err_create');
    } finally {
        setupLoading.value = false;
    }
}
...
async function updateStatus(status: 'active' | 'paused' | 'completed') {
    ...
    try {
        ...
        error.value =
            e instanceof Error ? e.message : t('story_relay.err_update');
    } finally {
...
async function submitPlayerTurn() {
    if (!selected.value || !playerTurnContent.value.trim()) {
        return;
    }
    ...
    try {
        ...
        error.value =
            e instanceof Error ? e.message : t('story_relay.err_submit');
    } finally {
...
async function aiGenerateChar() {
    ...
    try {
        const res = await fetchJson<{ character: Record<string, unknown> }>(
            api.characters.aiGenerate(),
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    description: charGenDesc.value,
                    genre: charGenGenre.value,
                }),
            },
        );
        ...
        charError.value =
            e instanceof Error ? e.message : t('story_relay.err_ai_generate');
    } finally {
        charLoading.value = false;
    }
}
...
async function saveChar() {
    ...
    try {
        const updated = await fetchJson<Character>(
            api.characters.update(selectedChar.value.id),
            {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
        ...
        charError.value =
            e instanceof Error ? e.message : t('story_relay.err_save');
    } finally {
...
async function aiRefineChar() {
    ...
    try {
        const res = await fetchJson<{ character: Record<string, unknown> }>(
            api.characters.aiRefine(),
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    character: charDraft.value,
                    notes: charRefineNotes.value,
                }),
            },
        );
        const c = res.character;
        const app =
            (c.appearance as CharDraft['appearance'] | null) ??
            charDraft.value.appearance;
        charDraft.value = {
...
async function generateImagePrompt() {
    ...
    try {
        const res = await fetchJson<{ image_prompt: string }>(
            api.characters.imagePrompt(selectedChar.value.id),
            {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        if (charDraft.value) {
            charDraft.value.image_prompt = res.image_prompt;
        }
...
async function deleteChar() {
    ...
    if (
        !confirm(
            t('story_relay.confirm_delete', { name: selectedChar.value.name }),
        )
    ) {
        return;
    }
    ...
    try {
        await fetchJson(api.characters.destroy(selectedChar.value.id), {
            method: 'DELETE',
        });
        ...
        charError.value =
            e instanceof Error ? e.message : t('story_relay.err_delete');
    } finally {
        charLoading.value = false;
    }
}
...
const genreOptions = computed(() =>
    storyGenres.value.map((v) => ({
        value: v,
        label: t(`story_relay.genre_${v}`),
    })),
);
...
<template>
    <AppLayout>
        ...
        >
            <!-- ── Tab bar ───────────────────────────────────── -->
            <div
                class="flex shrink-0 border-b border-[var(--binary-outline)]/20"
            >
                <button
                    v-for="tab in [
                        { key: 'story', label: t('story_relay.tab_story') },
                        {
                            key: 'characters',
                            label: t('story_relay.tab_characters'),
                        },
                    ]"
                    :key="tab.key"
                    type="button"
                    class="px-5 py-2.5 text-xs tracking-widest uppercase transition"
                    :class="
                        mainTab === tab.key
            ...
            <div v-if="mainTab === 'story'" class="flex flex-1 overflow-hidden">
                ...
                >
                    <div class="border-b border-[var(--binary-outline)]/20 p-4">
                        <button
                            class="binary-ghost-button w-full py-2 text-xs tracking-widest uppercase"
                            type="button"
                            @click="openSetup"
                        >
                    ...
                    <div class="flex-1 overflow-y-auto p-2">
                        <p
                            v-if="isLoadingList"
                            class="p-4 text-center text-xs text-[var(--binary-text-muted)]"
                        >
                            {{ t('story_relay.loading') }}
                        </p>
                        <p
                            v-else-if="!sessions.length"
                            class="p-4 text-center text-xs text-[var(--binary-text-muted)]"
                        >
                            {{ t('story_relay.no_stories') }}
                        </p>
                        <button
                            v-for="s in sessions"
                            :key="s.id"
                ...
                >
                    ...
                    >
                        <p
                            class="binary-label text-xs text-[var(--binary-text-muted)] uppercase"
                        >
                            {{ t('story_relay.empty_select') }}
                        </p>
                    </div>

                    <!-- Setup area -->
                    <div
                        v-else-if="panel === 'setup'"
                    ...
                    >
                        ...
                        <template v-if="setupStep === 0">
                            ...
                            <textarea
                                ...
                                rows="3"
                                :placeholder="
                                    t('story_relay.setup_keywords_placeholder')
                                "
                                maxlength="200"
                            />
                            <div class="flex justify-end">
                                ...
                                >
                                    {{
                                        setupLoading
                                            ? t('story_relay.setup_generating')
                                            : t('story_relay.setup_generate')
                                    }}
                                </button>
                            </div>
                        </template>
                        ...
                        <template v-else-if="setupStep === 1 && setupDraft">
                            <div class="mb-4 space-y-4">
                                <div>
                                    <p
                                        class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                    >
                                        {{ t('story_relay.setup_world') }}
                                    </p>
                                    <textarea
                                ...
                                <div>
                                    ...
                                    >
                                        {{
                                            t(
                                                'story_relay.setup_characters_label',
                                            )
                                        }}
                                    </p>
                                    <div
                                        v-for="(c, i) in setupDraft.characters"
                                        :key="i"
                                        class="binary-card-raised mb-2 rounded-lg p-3"
                                    >
                                        <div
                                        ...
                                        >
                                            ...
                                            <button
                                                type="button"
                                                class="shrink-0 rounded px-2 py-1 text-[10px] tracking-widest uppercase transition"
                                                :class="
                                                    c.is_narrator !== false
                                            ...
                                            >
                                                {{
                                                    c.is_narrator !== false
                                                        ? t(
                                                              'story_relay.btn_narrator',
                                                          )
                                                        : t(
                                                              'story_relay.btn_observer',
                                                          )
                                                }}
                                            </button>
                                        </div>
                                        <textarea
                                            v-model="c.persona"
                                            class="binary-input mb-1 w-full resize-none text-xs"
                                            rows="2"
                                            :placeholder="
                                                t(
                                                    'story_relay.setup_char_persona',
                                                )
                                            "
                                            maxlength="300"
                                        />
                                        <input
                                            v-model="c.secret"
                                            class="binary-input w-full text-xs"
                                            :placeholder="
                                                t(
                                                    'story_relay.setup_char_secret',
                                                )
                                            "
                                            maxlength="200"
                                        />
                                ...
                                <div v-if="setupDraft.items?.length">
                                    <p
                                        class="binary-label mb-2 text-[10px] text-[var(--binary-outline)] uppercase"
                                    >
                                        {{ t('story_relay.setup_items_label') }}
                                    </p>
                                    <div
                                        v-for="(item, i) in setupDraft.items"
                                        :key="i"
                                        class="binary-card-raised mb-2 rounded-lg p-3"
                                    >
                                        <input
                                            v-model="item.name"
                                            class="binary-input mb-1 w-full text-xs"
                                            :placeholder="
                                                t('story_relay.setup_item_name')
                                            "
                                            maxlength="100"
                                        />
                                        <input
                            ...
                            <div class="flex gap-3">
                                ...
                                >
                                    {{
                                        setupLoading
                                            ? t('story_relay.btn_refining')
                                            : t('story_relay.btn_refine')
                                    }}
                                </button>
                                <button
                                    ...
                                    @click="setupStep = 2"
                                >
                                    {{ t('story_relay.btn_confirm_setup') }}
                                </button>
                            </div>
                        </template>
                        ...
                        <template v-else-if="setupStep === 2">
                            <div class="mb-4 space-y-4">
                                <div>
                                    <p
                                        class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                    >
                                        {{ t('story_relay.setup_story_title') }}
                                    </p>
                                    <input
                                        v-model="setupTitle"
                                        ...
                                        :placeholder="
                                            t(
                                                'story_relay.setup_title_placeholder',
                                            )
                                        "
                                    />
                                </div>
                                <div class="flex gap-6">
                                    ...
                                    <div>
                                        <p
                                            class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                        >
                                            {{ t('story_relay.setup_rounds') }}
                                        </p>
                                        <input
                                            v-model.number="
                                                setupRoundsPerAdvance
                                            "
                                            type="number"
                                            min="1"
                                            max="10"
                                            class="binary-input w-20"
                                        />
                                    </div>
                                </div>
                                <div>
                            ...
                            <div class="flex gap-3">
                                <button
                                    class="binary-ghost-button px-4 py-2 text-xs opacity-60"
                                    type="button"
                                    @click="setupStep = 1"
                                >
                                    {{ t('story_relay.btn_back') }}
                                ...
                                <button
                                    ...
                                    type="button"
                                    :disabled="
                                        setupLoading || !setupTitle.trim()
                                    "
                                    @click="createSession"
                                >
                                    {{
                                        setupLoading
                                            ? t('story_relay.btn_creating')
                                            : t('story_relay.btn_create')
                                    }}
                                </button>
                            </div>
                        </template>
                    ...
                    <template v-else-if="panel === 'session' && selected">
                        ...
                        <div
                            v-if="isAdmin"
                            class="flex items-center gap-3 border-b border-[var(--binary-outline)]/20 bg-[var(--binary-surface)] px-5 py-3"
                        >
                            <span
                                class="binary-label text-[10px] uppercase"
                                :class="statusColor[selected.status]"
                            >
                            ...
                            <div class="ml-auto flex gap-2">
                                ...
                                >
                                    {{ t('story_relay.btn_pause') }}
                                </button>
                                <button
                                    v-if="selected.status !== 'completed'"
                                    class="binary-ghost-button px-3 py-1 text-[10px] text-red-400/60 uppercase hover:text-red-300 disabled:opacity-40"
                                    :disabled="isUpdatingStatus"
                                    type="button"
                                    @click="updateStatus('completed')"
                                >
                                    {{ t('story_relay.btn_end') }}
                                </button>
                            </div>
                        </div>
                        ...
                        <div
                            v-if="isPlayerTurn()"
                            class="border-b border-[var(--binary-outline)]/20 bg-[var(--binary-surface-container)] px-5 py-3"
                        >
                            <p
                                class="binary-label mb-2 text-[10px] text-[var(--binary-primary)] uppercase"
                            >
                                {{
                                    t('story_relay.player_turn', {
                                        name: selected.current_character?.name,
                                    })
                                }}
                            </p>
                            <textarea
                                ...
                                rows="3"
                                :placeholder="t('story_relay.turn_placeholder')"
                                maxlength="1000"
                            />
                            <div class="flex justify-end">
                                <button
                                    ...
                                    :disabled="
                                        isSubmittingTurn ||
                                        !playerTurnContent.trim()
                                    "
                                    @click="submitPlayerTurn"
                                >
                                    {{
                                        isSubmittingTurn
                                            ? t('story_relay.btn_submitting')
                                            : t('story_relay.btn_submit')
                                    }}
                                </button>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :class="
                                        seg.is_event
                                            ? 'border border-amber-400/20 bg-amber-950/10'
                                            : ''
                                    "
                                >
                                    <div class="mb-2 flex items-center gap-2">
                                        ...
                                        >
                                            {{
                                                seg.is_event
                                                    ? t(
                                                          'story_relay.event_label',
                                                      )
                                                    : (seg.character?.name ??
                                                      t(
                                                          'story_relay.narrator_label',
                                                      ))
                                            }}
                                        </span>
                                        <span
                                            class="text-[10px] text-[var(--binary-text-muted)]"
                                            >#{{ seg.turn_number }}</span
                                        >
                                        <span
                                            v-if="seg.is_player_written"
                                            class="text-[10px] text-[var(--binary-primary)]"
                                            >{{
                                                t('story_relay.player_badge')
                                            }}</span
                                        >
                                    </div>
                                    <p
                            ...
                            >
                                <div>
                                    ...
                                    >
                                        {{
                                            t('story_relay.label_next_advance')
                                        }}
                                    </p>
                                    <p
                                        class="text-xs"
                                    ...
                                    >
                                        {{
                                            ...
                                            (selected.status === 'active'
                                                ? t(
                                                      'story_relay.countdown_calculating',
                                                  )
                                                : t(
                                                      'story_relay.countdown_paused',
                                                  ))
                                        }}
                                    </p>
                                </div>
                                <div>
                                    ...
                                    >
                                        {{
                                            t(
                                                'story_relay.label_characters_sidebar',
                                            )
                                        }}
                                    </p>
                                    <div class="space-y-1.5">
                                        ...
                                        >
                                            <span
                                                class="mt-px shrink-0 text-[10px]"
                                                >{{
                                                    c.id ===
                                                        selected.current_character_id &&
                                                    selected.status === 'active'
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p
                                                    ...
                                                    class="text-[10px] text-red-400/70"
                                                >
                                                    {{ c.status }}
                                                </p>
                                            </div>
                                ...
                                <div v-if="selected.items.length">
                                    ...
                                    >
                                        {{
                                            t('story_relay.label_items_sidebar')
                                        }}
                                    </p>
                                    <div class="space-y-1.5">
                                        <div
                                        ...
                                        >
                                            <p
                                                class="font-semibold text-[var(--binary-text)]"
                                            >
                                                {{ item.name }}
                                            </p>
                                            <p class="text-[10px]">
                                ...
                                <div v-if="selected.world_state">
                                    <p
                                        class="binary-label mb-2 text-[10px] text-[var(--binary-outline)] uppercase"
                                    >
                                        {{ t('story_relay.label_world_state') }}
                                    </p>
                                    <p
                                        class="text-[10px] leading-relaxed whitespace-pre-wrap text-[var(--binary-text-muted)]"
                                    >
                                        {{ worldStateSummary }}
                                    </p>
                                    <button
                                        v-if="selected.world_state.length > 200"
                                        class="mt-1 text-[10px] text-[var(--binary-outline)] hover:text-[var(--binary-text)]"
                                        type="button"
                                        @click="
                                            worldStateExpanded =
                                                !worldStateExpanded
                                    ...
                                    >
                                        {{
                                            worldStateExpanded
                                                ? t('story_relay.btn_collapse')
                                                : t('story_relay.btn_expand')
                                        }}
                                    </button>
                                </div>
                            </aside>
            ...
            <div v-else class="flex flex-1 overflow-hidden">
                ...
                >
                    ...
                    <div class="flex-1 overflow-y-auto p-2">
                        <p
                            v-if="charLoading && !characters.length"
                            class="p-4 text-center text-xs text-[var(--binary-text-muted)]"
                        >
                            {{ t('story_relay.loading') }}
                        </p>
                        ...
                        >
                            ...
                            <p
                                class="mt-0.5 truncate text-[10px] text-[var(--binary-text-muted)]"
                            >
                                {{ c.persona.slice(0, 60)
                                }}{{ c.persona.length > 60 ? '…' : '' }}
                            </p>
                        </button>
                    </div>
                ...
                >
                    ...
                    >
                        <p
                            class="binary-label text-xs text-[var(--binary-text-muted)] uppercase"
                        >
                            {{ t('story_relay.empty_char_select') }}
                        </p>
                    </div>
                    ...
                    <div v-else-if="charView === 'generate'" class="p-6">
                        <h2
                            class="binary-label mb-6 text-xs tracking-widest text-[var(--binary-outline)] uppercase"
                        >
                            {{ t('story_relay.char_generate_title') }}
                        </h2>
                        <p
                            v-if="charError"
                            class="mb-4 rounded-lg border border-red-400/20 bg-red-950/20 px-4 py-2 text-xs text-red-300"
                        ...
                        <div class="mb-4 grid grid-cols-4 gap-2">
                            <button
                                ...
                                class="rounded-lg border px-3 py-2 text-xs transition"
                                :class="
                                    charGenGenre === g.value
                                        ? 'border-[var(--binary-primary)] text-[var(--binary-primary)]'
                                        : 'border-[var(--binary-outline)]/30 text-[var(--binary-text-muted)] hover:border-[var(--binary-outline)]'
                                "
                                @click="
                                    charGenGenre =
                                        g.value as typeof charGenGenre
                                "
                            >
                                {{ g.label }}
                            </button>
                        ...
                        <textarea
                            ...
                            rows="4"
                            :placeholder="
                                t('story_relay.char_desc_placeholder')
                            "
                            maxlength="500"
                        />
                        <div class="flex justify-end">
                            <button
                                ...
                                @click="aiGenerateChar"
                            >
                                {{
                                    charLoading
                                        ? t('story_relay.setup_generating')
                                        : t('story_relay.btn_ai_generate')
                                }}
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Edit -->
                    <div
                        v-else-if="charView === 'edit' && charDraft"
                        class="p-6"
                    >
                        <div class="mb-5 flex items-center justify-between">
                            <h2
                                class="binary-label text-xs tracking-widest text-[var(--binary-outline)] uppercase"
                            >
                                {{ t('story_relay.char_edit_title') }}
                            </h2>
                            <button
                                class="text-[10px] text-red-400/60 transition hover:text-red-300 disabled:opacity-40"
                                type="button"
                                :disabled="charLoading"
                                @click="deleteChar"
                            >
                                {{ t('story_relay.btn_delete') }}
                            </button>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <p
                                    class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_persona') }}
                                </p>
                                <textarea
                                    v-model="charDraft.persona"
                                    class="binary-input w-full resize-none"
                                    rows="3"
                                    maxlength="500"
                                />
                            ...
                            <div>
                                <p
                                    class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_background') }}
                                </p>
                                <textarea
                                    v-model="charDraft.background"
                                    class="binary-input w-full resize-none"
                                    rows="2"
                                    maxlength="500"
                                />
                            ...
                            <div>
                                <p
                                    class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_secret') }}
                                </p>
                                <textarea
                                    v-model="charDraft.secret"
                                    class="binary-input w-full resize-none"
                                    rows="2"
                                    maxlength="500"
                                />
                            ...
                            <div>
                                <p
                                    class="binary-label mb-2 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_appearance') }}
                                </p>
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <p
                                            class="mb-1 text-[10px] text-[var(--binary-text-muted)]"
                                        >
                                            {{
                                                t('story_relay.appearance_age')
                                            }}
                                        </p>
                                        <input
                                            v-model="charDraft.appearance.age"
                                            class="binary-input w-full text-xs"
                                            maxlength="50"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        >
                                            {{
                                                t('story_relay.appearance_hair')
                                            }}
                                        </p>
                                        <input
                                            v-model="charDraft.appearance.hair"
                                            class="binary-input w-full text-xs"
                                            maxlength="100"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        >
                                            {{
                                                t('story_relay.appearance_eyes')
                                            }}
                                        </p>
                                        <input
                                            v-model="charDraft.appearance.eyes"
                                            class="binary-input w-full text-xs"
                                            maxlength="100"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        >
                                            {{
                                                t(
                                                    'story_relay.appearance_build',
                                                )
                                            }}
                                        </p>
                                        <input
                                            v-model="charDraft.appearance.build"
                                            class="binary-input w-full text-xs"
                                            maxlength="100"
                                        />
                                    </div>
                                    <div class="col-span-2">
                                        ...
                                        >
                                            {{
                                                t(
                                                    'story_relay.appearance_features',
                                                )
                                            }}
                                        </p>
                                        <input
                                            v-model="
                                                charDraft.appearance.features
                                            "
                                            class="binary-input w-full text-xs"
                                            maxlength="200"
                                        />
                            ...
                            <div>
                                <p
                                    class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_outfit') }}
                                </p>
                                <input
                                    v-model="charDraft.outfit"
                                    class="binary-input w-full"
                                    maxlength="300"
                                />
                            </div>
                            <div>
                                <p
                                    class="binary-label mb-1 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_image_prompt') }}
                                </p>
                                <textarea
                                    v-model="charDraft.image_prompt"
                                    class="binary-input w-full resize-none text-[10px] text-[var(--binary-text-muted)]"
                                    rows="3"
                                />
                                <button
                                ...
                                >
                                    {{
                                        charImgLoading
                                            ...
                                            : t(
                                                  'story_relay.btn_ai_image_prompt',
                                              )
                                    }}
                                </button>
                            </div>
                            ...
                            <!-- AI refine -->
                            <div
                                class="rounded-lg border border-[var(--binary-outline)]/20 p-3"
                            >
                                <p
                                    class="binary-label mb-2 text-[10px] text-[var(--binary-outline)] uppercase"
                                >
                                    {{ t('story_relay.label_ai_refine') }}
                                </p>
                                <input
                                    v-model="charRefineNotes"
                                    class="binary-input mb-2 w-full text-xs"
                                    :placeholder="
                                        t('story_relay.refine_placeholder')
                                    "
                                    maxlength="300"
                                />
                                <button
                                    class="binary-ghost-button px-4 py-1.5 text-[10px] disabled:opacity-40"
                                    type="button"
                                    :disabled="charLoading"
                                    @click="aiRefineChar"
                                >
                                    {{
                                        charLoading
                                            ? t('story_relay.btn_refining')
                                            : t(
                                                  'story_relay.btn_ai_refine_char',
                                              )
                                    }}
                                </button>
                            </div>
                        ...
                        <div class="mt-6 flex justify-end">
                            ...
                            >
                                {{
                                    charSaving
                                        ? t('story_relay.btn_saving')
                                        : t('story_relay.btn_save')
                                }}
                            </button>
                        </div>
                    </div>
