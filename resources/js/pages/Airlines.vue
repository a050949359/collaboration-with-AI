    ...
    <AppLayout>
        ...
        <div class="mx-auto max-w-screen-xl px-6 pb-24 pt-32 md:px-8">
            ...
            <div class="mb-10 pt-8">
                <span class="binary-label mb-2 block text-xs font-bold uppercase text-[var(--binary-primary)]">&gt; airline_database</span>
                <h1 class="binary-display text-5xl font-black uppercase tracking-tight md:text-7xl">{{ t('airlines.title').toUpperCase() }}</h1>
                <p class="mt-3 text-sm text-[var(--binary-text-muted)]">{{ t('airlines.subtitle') }}</p>
            ...
            <!-- Table -->
            <template v-else>
                <div v-if="airlines.length === 0" class="py-20 text-center binary-label text-sm text-[var(--binary-outline)]">
                    {{ t('common.no_data') }}
                </div>
                ...
                <div v-else class="overflow-x-auto rounded-xl border border-[var(--binary-outline-variant)]">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-[var(--binary-outline-variant)]">
                            ...
                            >
                                <td class="px-4 py-3 font-mono text-[var(--binary-primary)] font-bold">
                                    {{ airline.iata ?? '—' }}
                                </td>
                                <td class="px-4 py-3 font-mono text-[var(--binary-outline)]">
                                    {{ airline.icao ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-[var(--binary-text)]">{{ airline.name_en }}</td>
                                <td class="px-4 py-3 text-[var(--binary-text)]">{{ airline.name_zh_tw ?? '—' }}</td>
                                <td class="px-4 py-3 text-[var(--binary-outline)]">{{ airline.nationality ?? '—' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
