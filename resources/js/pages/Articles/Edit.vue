...
async function loadArticle() {
    ...
    try {
        ...
        errorMessage.value = error instanceof ArticleApiError ? error.message : t('articles.edit.load_failed');
    } finally {
...
<template>
    <Head :title="article?.title ? `${article.title} / ${t('articles.edit.head_title_suffix')}` : t('articles.edit.head_title_fallback')" />
    ...
    <AppLayout>
        <main class="pt-24 pb-24">
            <section class="mx-auto max-w-screen-xl px-6 py-12 md:px-8">
                ...
                <p v-else-if="errorMessage && !article"
                ...
                <template v-else-if="article">
                    ...
                    <div class="binary-card-raised mb-6 rounded-2xl">
                        ...
                        <p class="text-sm text-[var(--binary-text-muted)]">
                            {{ t('articles.edit.category') }}: {{ article.category ?? '-' }}
                        </p>
                    </div>
                    ...
                    <div class="binary-card-raised rounded-2xl space-y-6">
                        ...
                        <div>
                            <label class="binary-label mb-1 block text-[10px] uppercase text-[var(--binary-outline)]">{{ t('articles.edit.field_title') }}</label>
                            <input
                                v-model="title"
                                type="text"
                                maxlength="255"
                                :placeholder="t('articles.edit.title_placeholder')"
                                class="binary-input w-full"
                            />
                        </div>

                        <div>
                            <label class="binary-label mb-1 block text-[10px] uppercase text-[var(--binary-outline)]">{{ t('articles.edit.field_summary') }}</label>
                            <textarea
                                v-model="summary"
                                rows="3"
                                maxlength="500"
                                :placeholder="t('articles.edit.summary_placeholder')"
                                class="binary-input w-full resize-none"
                            />
                            <p class="mt-1 text-right text-[10px] text-[var(--binary-text-muted)]">
                                {{ summary.length }} / 500
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="binary-label mb-1 block text-[10px] uppercase text-[var(--binary-outline)]">
                                {{ t('articles.edit.field_tags') }} <span class="normal-case">{{ t('articles.edit.field_tags_hint') }}</span>
                            </label>
                            <input
                                v-model="tagsRaw"
                                type="text"
                                :placeholder="t('articles.edit.tags_placeholder')"
                                class="binary-input w-full"
                            />
                        </div>

                        <div>
                            <label class="binary-label mb-1 block text-[10px] uppercase text-[var(--binary-outline)]">{{ t('articles.edit.field_content') }}</label>
                            <textarea
                                v-model="content"
                                rows="20"
                                :placeholder="t('articles.edit.content_placeholder')"
                        ...
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                @click="save"
                            >
                                {{ isSaving ? t('articles.edit.saving') : t('articles.edit.save') }}
                            </button>
                        </div>
