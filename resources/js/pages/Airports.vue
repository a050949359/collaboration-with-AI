        ...
        <main class="pt-24 pb-24">
            <div class="mx-auto max-w-screen-2xl px-6 md:px-8">
                ...
                <div class="mb-10 pt-8">
                    <span class="binary-label mb-2 block text-xs font-bold uppercase text-[var(--binary-primary)]">&gt; airport_database</span>
                    <h1 class="binary-display text-5xl font-black uppercase tracking-tight md:text-7xl">{{ t('airports.title').toUpperCase() }}</h1>
                ...
                <template v-if="activeTab === 'search'">
                    ...
                    <!-- Error -->
                    <p v-if="listError" class="mb-4 rounded-lg border border-red-400/20 bg-red-950/20 px-4 py-3 text-sm text-red-300">
                        {{ listError }}
                    </p>

                    <!-- Loading -->
                    <div v-if="isLoadingList" class="py-16 text-center text-sm text-[var(--binary-text-muted)]">
                        {{ t('common.loading') }}
                    </div>

                    <!-- Empty -->
                    <div v-else-if="!airports.length" class="py-16 text-center text-sm text-[var(--binary-text-muted)]">
                        {{ t('common.no_data') }}
                    </div>

                    <!-- Table -->
                    <template v-else>
                        <div class="overflow-x-auto rounded-2xl border border-[var(--binary-outline)]/10">
                        ...
                        <!-- Pagination -->
                        <div v-if="meta" class="mt-6 flex flex-col items-center justify-between gap-4 sm:flex-row">
                            <span class="text-xs text-[var(--binary-outline)]">
                                {{ t('common.total', { total: meta.total.toLocaleString(), current: meta.current_page, last: meta.last_page }) }}
                            </span>
                            <div class="flex gap-2">
                                <button
                                    class="binary-ghost-button px-4 py-1.5 text-xs disabled:opacity-30"
                                    :disabled="meta.current_page <= 1"
                                    @click="fetchAirports(meta!.current_page - 1)"
                                >
                                    {{ t('common.prev_page') }}
                                </button>
                                <button
                ...
                <template v-else-if="activeTab === 'stats'">
                    <div v-if="isLoadingStats" class="py-16 text-center text-sm text-[var(--binary-text-muted)]">
                        {{ t('airports.stats.loading') }}
                    </div>
                    <p v-else-if="statsError" class="rounded-lg border border-red-400/20 bg-red-950/20 px-4 py-3 text-sm text-red-300">
                        {{ statsError }}
                    </p>
                    ...
                    <template v-else-if="stats">
                        ...
                        <div class="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
                            <div class="binary-card-raised rounded-2xl text-center">
                                <div class="binary-display text-4xl font-black text-[var(--binary-primary)]">
                                    {{ stats.total.toLocaleString() }}
                                </div>
                                <div class="mt-1 binary-label text-[10px] uppercase text-[var(--binary-outline)]">{{ t('airports.stats.total') }}</div>
                            </div>
                            <div class="binary-card-raised rounded-2xl text-center">
                                <div class="binary-display text-4xl font-black text-[var(--binary-primary)]">
                                    {{ stats.scheduled_service.toLocaleString() }}
                                </div>
                                <div class="mt-1 binary-label text-[10px] uppercase text-[var(--binary-outline)]">{{ t('airports.stats.scheduled') }}</div>
                            </div>
                            <div class="binary-card-raised rounded-2xl text-center">
                                <div class="binary-display text-4xl font-black text-[var(--binary-primary)]">
                                    {{ Object.keys(stats.by_continent).length }}
                                </div>
                                <div class="mt-1 binary-label text-[10px] uppercase text-[var(--binary-outline)]">{{ t('airports.stats.continents') }}</div>
                            </div>
                            <div class="binary-card-raised rounded-2xl text-center">
                                <div class="binary-display text-4xl font-black text-[var(--binary-primary)]">
                                    {{ Object.keys(stats.top_countries).length }}+
                                </div>
                                <div class="mt-1 binary-label text-[10px] uppercase text-[var(--binary-outline)]">{{ t('airports.stats.countries') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div class="binary-card-raised rounded-2xl">
                                <h3 class="binary-label mb-4 text-xs font-bold uppercase text-[var(--binary-outline)]">&gt; {{ t('airports.stats.by_type') }}</h3>
                                <div class="space-y-3">
                                    <div v-for="[type, count] in topTypeEntries" :key="type">
                                        <div class="mb-1 flex items-center justify-between binary-label text-xs">
                                            <span class="text-[var(--binary-text)]">{{ t(`airports.types.${type}`, type) }}</span>
                                            <span class="text-[var(--binary-primary)]">{{ count.toLocaleString() }}</span>
                                        </div>
                                        <div class="h-1.5 w-full rounded-full bg-[var(--binary-surface-container)]">
                                            <div
                            ...
                            <!-- By Continent -->
                            <div class="binary-card-raised rounded-2xl">
                                <h3 class="binary-label mb-4 text-xs font-bold uppercase text-[var(--binary-outline)]">&gt; {{ t('airports.stats.by_continent') }}</h3>
                                <div class="space-y-2">
                                    <div
                                        v-for="[continent, count] in topContinentEntries"
                                        :key="continent"
                                        class="flex items-center justify-between rounded-lg bg-[var(--binary-surface-container)] px-3 py-2"
                                    >
                                        <span class="binary-label text-xs text-[var(--binary-text)]">
                                            {{ t(`airports.continents.${continent}`, continent) }}
                                        </span>
                                        <span class="binary-label text-xs font-bold text-[var(--binary-primary)]">
                            ...
                            <!-- Top Countries -->
                            <div class="binary-card-raised rounded-2xl md:col-span-2">
                                <h3 class="binary-label mb-4 text-xs font-bold uppercase text-[var(--binary-outline)]">&gt; {{ t('airports.stats.top_countries') }}</h3>
                                <div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
                                    <div
                                        v-for="([country, count], i) in topCountryEntries"
                                        :key="country"
                                        class="rounded-xl bg-[var(--binary-surface-container)] p-3 text-center"
                                    >
                                        <div class="binary-label text-[10px] text-[var(--binary-outline)]">#{{ i + 1 }}</div>
                                        <div class="binary-display mt-1 text-xl font-black text-[var(--binary-primary)]">{{ country }}</div>
                                        <div class="mt-0.5 binary-label text-xs text-[var(--binary-text-muted)]">{{ count.toLocaleString() }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
