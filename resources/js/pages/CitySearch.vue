...
async function fetchJobs() {
    ...
    const res = await fetch(`${api.cities.search.index()}?${params}`, {
        credentials: 'include',
    });
    const json = await res.json();
...
function startPolling(jobId: number) {
    ...
    pollTimer = window.setInterval(async () => {
        const res = await fetch(api.cities.search.show(jobId), {
            credentials: 'include',
        });
        const json = await res.json();
        const job = json.data;
...
<template>
    <AppLayout>
        ...
        <div class="mx-auto max-w-screen-md px-6 pt-32 pb-24 md:px-8">
            <div class="mb-8 pt-8">
                <span
            ...
            <div
                v-if="!user"
                class="rounded-xl border border-[var(--binary-outline-variant)] p-8 text-center text-sm text-[var(--binary-outline)]"
            >
            ...
            <template v-else>
                <!-- Tabs -->
                <div
                    class="mb-6 flex gap-4 border-b border-[var(--binary-outline-variant)]"
                >
                    <button
                ...
                <div v-if="activeTab === 'search'">
                    <div class="mb-6 flex gap-3">
                        <input
                            ...
                            placeholder="TW"
                            maxlength="2"
                            class="w-20 rounded-lg border border-[var(--binary-outline-variant)] bg-[var(--binary-surface-container)] px-3 py-2 font-mono text-sm text-[var(--binary-text)] uppercase placeholder:text-[var(--binary-outline)] focus:border-[var(--binary-primary)] focus:outline-none"
                        />
                        <input
                            v-model="cityName"
                            type="text"
                        ...
                        <button
                            ...
                            class="binary-label rounded-lg bg-[var(--binary-surface-container)] px-4 py-2 text-xs text-[var(--binary-primary)] uppercase transition hover:bg-[var(--binary-surface-high)] disabled:opacity-40"
                            @click="search"
                        >
                            {{
                                isSearching
                    ...
                    <div v-if="candidates.length" class="space-y-3">
                        ...
                        >
                            ...
                            <button
                                ...
                                class="binary-label flex-shrink-0 rounded-lg bg-[var(--binary-primary)]/10 px-3 py-1.5 text-[10px] text-[var(--binary-primary)] uppercase transition hover:bg-[var(--binary-primary)]/20 disabled:opacity-40"
                                @click="confirm(c)"
                            >
                ...
                <div v-else>
                    <div
                        v-if="jobs.length === 0"
                        class="py-16 text-center text-sm text-[var(--binary-outline)]"
                    >
                        {{ t('common.no_data') }}
                    </div>
