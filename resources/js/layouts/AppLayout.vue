...
onMounted(() => {
    ...
    interface Stream {
        col: number; size: number; speed: number; alpha: number;
        yOffset: number; textLen: number; y: number; history: string[];
    }
    ...
    function makeStream(col: number, cosVal: number, ratio: number): Stream {
        ...
        return {
            ...
            textLen,
            y: (Math.random() - 1) * 80,
            history: Array.from({ length: textLen }, () => CHARS[Math.floor(Math.random() * CHARS.length)]),
        };
    }
    ...
    function init() {
        ...
        while (i < columns) {
            ...
            const step = Math.max(1, Math.round(arcSize(Math.abs(ratio)) / COL_W));
    ...
    function loop() {
        ...
        for (const s of streams) {
            ...
            if (Math.random() < 0.008) {
                s.history[Math.floor(Math.random() * s.textLen)] = CHARS[Math.floor(Math.random() * CHARS.length)];
            }
            if (s.y * s.size + s.yOffset > canvas.height + 500) s.y = -s.textLen;
            ctx.font = `bold ${s.size}px monospace`;
            const x = s.col * COL_W;
            for (let i = 0; i < s.textLen; i++) {
                const y = (s.y - i) * s.size * 0.78 + canvas.height / 2 + s.yOffset;
