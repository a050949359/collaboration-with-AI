...
const defaultNavLinks = computed((): NavLink[] => {
    ...
    return [
        ...
        { label: 'LineBot', href: routes.linebot(), icon: 'linebot', active: path.startsWith(routes.linebot()) },
        {
            label: 'Lab',
            icon: 'lab',
            active: path.startsWith(routes.tourPlayground()) || path.startsWith(routes.miniOrch()) || path.startsWith(routes.wsLab()) || path.startsWith(routes.gacha()) || (isAdmin.value && path.startsWith(routes.storyRelay())),
            children: [
