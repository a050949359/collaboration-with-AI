...
{
    private const TOOLS = ['bg_run', 'bg_status', 'bg_log'];
    ...
    private function bgRun(mixed $id, array $args): JsonResponse
    {
        $prompt = $args['prompt'] ?? '';
    ...
    public function toolSchemas(): array
    {
        return [
            [
                'name'        => 'bg_run',
                'description' => '在本地微型主機上以 agy 執行一個背景工作（非同步）。回傳 task_id，可用 bg_status 輪詢進度。',
                'inputSchema' => [
                    'type'       => 'object',
                    'properties' => [
            ...
            [
                ...
                'inputSchema' => [
                    'type'       => 'object',
                    'properties' => [
                        'task_id' => ['type' => 'string', 'description' => '由 bg_run 回傳的 task ID'],
                    ],
                    'required' => ['task_id'],
                ],
            ],
